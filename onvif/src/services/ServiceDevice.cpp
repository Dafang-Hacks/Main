...
int DeviceBindingService::GetServices(_tds__GetServices *tds__GetServices, _tds__GetServicesResponse *tds__GetServicesResponse)
{
		...
		{
			tds__GetServicesResponse->Service.back()->Capabilities = soap_new__tds__Service_Capabilities(this->soap);
			tmd__Capabilities *capabilities = getDeviceIOServiceCapabilities(this->soap);
		...
		{
			tds__GetServicesResponse->Service.back()->Capabilities = soap_new__tds__Service_Capabilities(this->soap);
			trv__Capabilities *capabilities = getReceiverServiceCapabilities(this->soap);
...
int DeviceBindingService::GetDeviceInformation(_tds__GetDeviceInformation *tds__GetDeviceInformation, _tds__GetDeviceInformationResponse *tds__GetDeviceInformationResponse)
{
		...
		ServiceContext::DeviceInformation device_info = ServiceContext::GetInstance()->get_device_info();

		tds__GetDeviceInformationResponse->Manufacturer    = device_info.Manufacturer;
		tds__GetDeviceInformationResponse->Model           = device_info.Model;
		tds__GetDeviceInformationResponse->FirmwareVersion = device_info.FirmwareVersion;
		tds__GetDeviceInformationResponse->SerialNumber    = device_info.SerialNumber;
		tds__GetDeviceInformationResponse->HardwareId      = device_info.HardwareId;
...
int DeviceBindingService::GetScopes(_tds__GetScopes *tds__GetScopes, _tds__GetScopesResponse *tds__GetScopesResponse)
{
		...
		for( auto scope : ServiceContext::GetInstance()->get_scopes() )
		{
			tds__GetScopesResponse->Scopes.push_back(
				soap_new_req_tt__Scope(soap, tt__ScopeDefinition__Fixed, scope)
			);
		}
...
int DeviceBindingService::GetCapabilities(_tds__GetCapabilities *tds__GetCapabilities, _tds__GetCapabilitiesResponse *tds__GetCapabilitiesResponse)
{
		...
		std::string XAddr = ServiceContext::GetInstance()->getXAddr(this->soap);

		tt__Capabilities *Capabilities = soap_new_tt__Capabilities(this->soap);

		Capabilities->Device = soap_new_tt__DeviceCapabilities(this->soap);
		Capabilities->Device->XAddr = XAddr;

		Capabilities->Device->Network = soap_new_tt__NetworkCapabilities(this->soap);
		Capabilities->Device->Network->IPFilter = soap_new_ptr(this->soap, false);
		Capabilities->Device->Network->ZeroConfiguration = soap_new_ptr(this->soap, false);
		Capabilities->Device->Network->IPVersion6 = soap_new_ptr(this->soap, false);
		Capabilities->Device->Network->DynDNS = soap_new_ptr(this->soap, false);

		Capabilities->Device->System = soap_new_tt__SystemCapabilities(this->soap);
		Capabilities->Device->System->DiscoveryResolve = true;
		Capabilities->Device->System->DiscoveryBye = true;
		Capabilities->Device->System->RemoteDiscovery = true;
		Capabilities->Device->System->SystemBackup = false;
		Capabilities->Device->System->SystemLogging = false;
		Capabilities->Device->System->FirmwareUpgrade = false;
		Capabilities->Device->System->SupportedVersions.push_back(soap_new_req_tt__OnvifVersion(this->soap, 1, 2));

		// Capabilities->Device->Security = soap_new_tt__SecurityCapabilities(this->soap);
		// Capabilities->Device->IO = soap_new_tt__IOCapabilities(this->soap);

		Capabilities->Events = soap_new_tt__EventCapabilities(this->soap);
		Capabilities->Events->XAddr = XAddr;
		Capabilities->Events->WSSubscriptionPolicySupport = false;
		Capabilities->Events->WSPullPointSupport = true;
		Capabilities->Events->WSPausableSubscriptionManagerInterfaceSupport = false;

		Capabilities->Imaging = soap_new_tt__ImagingCapabilities(this->soap);
		Capabilities->Imaging->XAddr = XAddr;

		Capabilities->Media = soap_new_tt__MediaCapabilities(this->soap);
		Capabilities->Media->XAddr = XAddr;
		Capabilities->Media->StreamingCapabilities = soap_new_tt__RealTimeStreamingCapabilities(this->soap);
		Capabilities->Media->StreamingCapabilities->RTPMulticast = soap_new_ptr(this->soap, false);
		Capabilities->Media->StreamingCapabilities->RTP_USCORETCP = soap_new_ptr(this->soap, false);
		Capabilities->Media->StreamingCapabilities->RTP_USCORERTSP_USCORETCP = soap_new_ptr(this->soap, true);

		if( ServiceContext::GetInstance()->has_ptz() ) {
			Capabilities->PTZ = soap_new_tt__PTZCapabilities(this->soap);
			Capabilities->PTZ->XAddr = XAddr;
		}

		Capabilities->Extension = soap_new_tt__CapabilitiesExtension(this->soap);

		Capabilities->Extension->DeviceIO = soap_new_req_tt__DeviceIOCapabilities(this->soap, XAddr, 1, 2, 1, 1, 0);

		Capabilities->Extension->Receiver = soap_new_req_tt__ReceiverCapabilities(this->soap, XAddr, false, true, true, 10, 256);

		tds__GetCapabilitiesResponse->Capabilities = Capabilities;//getDeviceServiceCapabilities(this->soap);
...
int DeviceBindingService::GetNetworkInterfaces(_tds__GetNetworkInterfaces *tds__GetNetworkInterfaces, _tds__GetNetworkInterfacesResponse *tds__GetNetworkInterfacesResponse)
{
	...
	// ServiceContext* ctx = (ServiceContext*)this->soap->user;
	std::vector<Eth_Dev_Param> interfaces = ServiceContext::GetInstance()->get_net_interfaces();
	...
	for(size_t i = 0; i < interfaces.size(); ++i)
	{
		...
		tds__GetNetworkInterfacesResponse->NetworkInterfaces.back()->Info->Name->assign(interfaces[i].dev_name());

		interfaces[i].get_hwaddr(tmp_buf);
		...
		tds__GetNetworkInterfacesResponse->NetworkInterfaces.back()->IPv4->Enabled = true;
		...
		interfaces[i].get_ip(tmp_buf);

		tds__GetNetworkInterfacesResponse->NetworkInterfaces.back()->IPv4->Config->Manual.back()->Address = tmp_buf;
		tds__GetNetworkInterfacesResponse->NetworkInterfaces.back()->IPv4->Config->Manual.back()->PrefixLength = interfaces[i].get_mask_prefix();
