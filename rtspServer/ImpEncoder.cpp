...
static void* update_thread(void *p) {
    ...
    while (true) {
        ...
        if (currentConfig.nightmode != newConfig->nightmode) {
            ...
            setNightVision(newConfig->nightmode);
        }
...
void ImpEncoder(impParams *params) 
{
...
int sample_encoder_init(int quality, int skiptype, int maxSameSceneCnt) {
...
*/
	...
	unsigned int size = 0;
        for (i = 0; i < nr_pack; i++) 
	{
	printf("Write %d len=%d\n", size, stream->pack[i].length);
		memcpy(jpeg_buffer + size, (void*) stream->pack[i].virAddr, stream->pack[i].length);
		size += stream->pack[i].length;
	}
	printf("Write %d\n", size);
