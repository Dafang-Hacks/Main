...
{			
	if (isCurrentlyAwaitingData()) 
	{
		...
		{				
			...
			//LOG_S(INFO) << "deliverFrame\ttimestamp:" << curTime.tv_sec << "." << curTime.tv_usec << "\tsize:" << fFrameSize <<"\tdiff:" <<  (diff.tv_sec*1000+diff.tv_usec/1000) << "ms\tqueue:" << m_captureQueue.size();		
...
int V4L2DeviceSource::getNextFrame() 
{
	...
	else
	{
		...
		//LOG_S(INFO) << "getNextFrame\ttimestamp:" << ref.tv_sec << "." << ref.tv_usec << "\tsize:" << frameSize <<"\tdiff:" <<  (diff.tv_sec*1000+diff.tv_usec/1000) << "ms";
...
void V4L2DeviceSource::processFrame(char * frame, int frameSize, const timeval &ref) 
{
	...
	while (!frameList.empty())
	{
		...
		//LOG_S(INFO) << "queueFrame\ttimestamp:" << ref.tv_sec << "." << ref.tv_usec << "\tsize:" << size <<"\tdiff:" <<  (diff.tv_sec*1000+diff.tv_usec/1000) << "ms";		
		frameList.pop_front();
	}			
}	
