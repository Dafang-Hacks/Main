	...
	if (isCurrentlyAwaitingData()) 
	{
		...
		{				
			...
			timersub(&curTime,&(frame->m_timestamp),&diff);
			 ...
			 LOG_S(9) << "deliverFrame\ttimestamp:" << curTime.tv_sec << "." << curTime.tv_usec << "\tsize:" << fFrameSize <<"\tdiff:" <<  (diff.tv_sec*1000+diff.tv_usec/1000) << "ms\tqueue:" << m_captureQueue.size();		
...
int V4L2DeviceSource::getNextFrame() 
{
    ...
    else 
    {
        ...
        LOG_S(9) << "getNextFrame\ttimestamp:" << ref.tv_sec << "." << ref.tv_usec << "\tsize:" << frameSize << "\tdiff:" << (diff.tv_sec * 1000 + diff.tv_usec / 1000) << "ms";
	    switch(m_outfd){
            case -1:
            break;
            case -2:
...
void V4L2DeviceSource::processFrame(char * frame, int frameSize, const timeval &ref) 
{
	...
	while (!frameList.empty())
	{
		...
		LOG_S(9) << "queueFrame\ttimestamp:" << ref.tv_sec << "." << ref.tv_usec << "\tsize:" << size <<"\tdiff:" <<  (diff.tv_sec*1000+diff.tv_usec/1000) << "ms";		
	}			
}	
...
void V4L2DeviceSource::queueFrame(char * frame, int frameSize, const timeval &tv) 
{
	...
	{
		LOG_S(9) << "Queue full size drop frame size:"  << (int)m_captureQueue.size() ;		
