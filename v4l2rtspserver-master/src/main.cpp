...
int main(int argc, char **argv, char**environ) {
    ...
    else 
    {
	    ...
	    {
		    OutPacketBuffer::maxSize = 950000;
	    } 
	    ...
	    if (disableAudio == false)
	    {
		    ...
		    if (audioCapture != NULL)
		    {
			    ...
			    else
			    {
				    ...
				    {
				         LOG_S(INFO) << "Audio streamer created (" <<rtpAudioFormat << ")" ;
				    }
				    else
				    {
				        LOG_S(FATAL) << "Unable to create audio streamer (" <<rtpAudioFormat << ")";
				    }
